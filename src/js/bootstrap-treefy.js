+function ($) {
	...
	Node.prototype.initIndent = function(treeFy) {
		...
		var depth = this.getDepth(treeFy.options);
	...
	Node.prototype.isCollapsed = function(treeContainer) {
		...
		if (isRoot) {
			...
			var parentNode = this.getParentNode(treeContainer);
			if (parentNode.row.hasClass('treetable-collapsed')) {
				return true;
			} else {
				return parentNode.isCollapsed(treeContainer);
			}
		}
	}
	...
	Node.prototype.getDepth = function(treeContainer) {
		...
		return parentNode.getDepth(treeContainer) + 1;
	}
	...
	Node.prototype.getParentNode = function(treeContainer)　{
		if (this.parentId === null) {
			...
			if (this.parentId == "") {
				return treeContainer.allNodesObject['undefined'];
			}
			return treeContainer.allNodesObject[this.parentId];
		}
	}
	...
	TreeFy.prototype.getAllNodes = function() {
		var self = this;
		var allNodes = [];
		$.each(self.$table.find('tr'), function(index, trElement) {
			var nodeData = $(trElement).attr('data-node');
			var template = /treetable-/;
			if (template.test(nodeData)) {
				var node = new Node($(trElement));
				node.addChildren(self.$table);
				allNodes.push(node);
				var id = nodeData.replace(template, '');
				self.options.allNodesObject[id] = node;
			}
		});
		
		var $row = self.$table.find('[data-node="treetable-"]');
		var node = new Node($row);
		node.addChildren(self.$table);
		self.options.allNodesObject['undefined'] = node;
	...
	TreeFy.prototype.render = function(nodes) {
		...
		$.each(nodes, function(node) {
			//若父节点折叠, 隐藏子节点
			if (this.isCollapsed(self.options)) {
				this.row.hide();
			} else {
				this.row.show();
			}
